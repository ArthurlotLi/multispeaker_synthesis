...
from speaker_encoder.data_objects.speaker_batch import SpeakerBatchSequential
...
class SpeakerVerificationDataset(Dataset):
  ...
  # How large this object is. Train for basically ever. 
  def __len__(self):
    return int(1e10)
...
# How we access the speaker objects. 
class SpeakerVerificationDatasetSequential(Dataset):
  # Expects the root datasets file and retains each dir as a speaker
  # object. 
  def __init__(self, datasets_root: Path):
    self.root = datasets_root
    speaker_dirs = [f for f in self.root.glob("*") if f.is_dir()]
    if len(speaker_dirs) == 0:
      raise Exception("SpeakerVerificationDatasetSequential - No speakers found! "
                      "Make sure you are pointing to the directory "
                      "containing all preprocessed speaker directories. ")
    self.num_speakers = len(speaker_dirs)
    self.speakers = [SpeakerSequential(speaker_dir) for speaker_dir in speaker_dirs]
    self.speaker_cycler = SequentialCycler(self.speakers)
  
  # How large this object is.
  def __len__(self):
    return self.num_speakers
  
  # Sequentially cycle to grab the next speaker. 
  def __getitem__(self, index):
    return next(self.speaker_cycler)
...
# The speaker labels. 
class SpeakerVerificationDataLoaderSequential(DataLoader):
  def __init__(self, dataset, speakers_per_batch, utterances_per_speaker, sampler=None, 
               batch_sampler=None, num_workers=0, pin_memory=False, timeout=0, 
               worker_init_fn=None):
    self.utterances_per_speaker = utterances_per_speaker
  ...
  # Combines a group of speakers into a batch object. 
  def collate(self, speakers):
    return SpeakerBatchSequential(speakers, self.utterances_per_speaker, partials_n_frames)